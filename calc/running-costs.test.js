...
const rates2 = { electricityCentsPerKwh: 28, otherRunningCostsAnnual: 1240, petrolCentsPerLitre: 195 };
...
test('a BEV burns no petrol whatever the battery share says', () => {
  ...
  const costs = runningCosts({ vehicle: bev, annualKm: 15000, batterySharePct: 40, rates: rates2 });
...
test('a PHEV at 100% battery share burns no petrol', () => {
  const costs = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 100, rates: rates2 });
...
test('a PHEV at 0% battery share runs entirely on petrol', () => {
  const costs = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 0, rates: rates2 });
...
test('the split is proportional to the battery share', () => {
  const costs = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 60, rates: rates2 });
...
test('battery share materially moves a PHEV total', () => {
  const mostlyElectric = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 80, rates: rates2 });
  const mostlyPetrol = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 20, rates: rates2 });
...
test('battery share defaults to fully electric, matching the BEV-only behaviour it replaced', () => {
  assert.deepEqual(
    runningCosts({ vehicle: phev, annualKm: 15000, rates: rates2 }),
    runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 100, rates: rates2 })
  );
});

test('an out-of-range battery share is clamped rather than producing negative kilometres', () => {
  const high = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: 140, rates: rates2 });
  const low = runningCosts({ vehicle: phev, annualKm: 15000, batterySharePct: -20, rates: rates2 });
